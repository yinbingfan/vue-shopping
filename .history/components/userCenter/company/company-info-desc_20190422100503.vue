  ...
  methods: {
    ...
    async getAgentList(param) {
      ...
      if (list.code === 200) {
        ...
        agentBasicResult.agentAddress = agentBasicResult.agentArea.replace(
          /\//g,
          ''
        )
        ...
        if (agentDetailResult.establishedTime) {
          agentDetailResult.foundTime = moment(
            agentDetailResult.establishedTime
          ).format('YYYY-MM-DD')
        } else {
    ...
    StrToGMT(time) {
      ...
      // let Ti= GMT.getFullYear()
      return GMT
    },
    editInfo() {
      const accountAuditStatus = JSON.parse(
        this.$cookie.get('accountAuditStatus')
      )
