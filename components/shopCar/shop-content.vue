...
<script>
  ...
  // import shopPagination from './shop-pagination.vue'
  ...
  export default {
    ...
    methods: {
      ...
      shopOrders(arr) {
        ...
        let cartIds=  JSON.stringify(arr)
        this.$cookie.set('shoppingCartIds', cartIds)
        let isSet=true
        this.orderGoTo([...new Set(arr)],isSet)
        // console.log('shoppingCartIds',JSON.parse(this.$cookie.get('shoppingCartIds')))
      },
      ...
      async orderSubmit(cartIds, addressId) {
        let shoppingCartIds=JSON.parse(this.$cookie.get('shoppingCartIds'))

        if(shoppingCartIds){
          cartIds=  shoppingCartIds
          this.$cookie.delete('shoppingCartIds')
        }
        if(addressId){
      ...
      async orderGoTo(ids,isSet) {
        if(isSet){
         let idArr=  JSON.stringify(ids)
          this.$cookie.set('cartIds', idArr)
        }else {
          let listId=JSON.parse(this.$cookie.get('cartIds'))
          ids=[...new Set(listId)]
        }
